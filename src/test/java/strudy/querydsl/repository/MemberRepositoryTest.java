...
import strudy.querydsl.entity.QMember;
...
class MemberRepositoryTest {
    ...
    @Test
    @DisplayName("")
    void querydslPredicate() throws Exception {
        Team teamA = new Team("teamA");
        Team teamB = new Team("teamB");
        em.persist(teamA);
        em.persist(teamB);
        ...
        em.persist(member4);
        //given
        Iterable<Member> member0 = memberRepository.findAll(member.age.between(10, 40).and(member.username.eq("member1")));

        for (Member member5 : member0) {
            System.out.println("member2 = " + member5);
        }
